...
using BBoxEntries = std::vector<BBoxTmp>;  // vector of triangle bounding boxes needed during BVH construction
...
BVHNode* Recurse(BBoxEntries& work, REPORTPRM(float pct = 0.0f) int depth = 0)
{
	...
	REPORT(float pctSpan = 11.0f / powf(3.0f, depth);)
...
int CountBoxes(BVHNode *root) {
	if (!root->IsLeaf()) {
		auto* p = dynamic_cast<BVHInner*>(root);
	...
	unsigned &idxTriList)
{
	...
	if (!root->IsLeaf()) { // inner node
		...
		auto* p = dynamic_cast<BVHLeaf*>(root);
		auto count = (unsigned)p->_triangles.size();
        ...
        for (auto triangle : p->_triangles)
        {
            g_triIndexList[idxTriList++] = triangle - pFirstTriangle;
        }
	}
}
...
void UpdateBoundingVolumeHierarchy(const std::string& filename)
{
	if (!g_pSceneBVH) {
		std::string BVHcacheFilename(filename);
		BVHcacheFilename += ".bvh";
		
		bool bUpdateBVH = false;
		
		if (std::filesystem::exists(filename) 
		 && std::filesystem::exists(BVHcacheFilename)) {
            auto plyFileTime = std::filesystem::last_write_time(filename);
            auto bvhFileTime = std::filesystem::last_write_time(BVHcacheFilename);

            bUpdateBVH = plyFileTime > bvhFileTime;
		}
